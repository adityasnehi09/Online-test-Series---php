...
if(isset($_REQUEST['id']))
{
	...
	$sql1=mysqli_query($con,"select no_of_question from all_test where uid='$examid'");
	...
	if($total_candidate>0)
	{
		...
		while($row=mysqli_fetch_assoc($sql))
		{
			...
			$sq=mysqli_query($con,"select * from all_test where uid='$examid' ");
			...
			if(mysqli_query($con,"update `$que_ans_list` set marks='$totalmarks_user',unsolved_que='$unsolved_que',negative_mark='$total_negative_marks' where user_id='$user_id'"))
			{
...
if(mysqli_query($con,"update all_test set rank_status='1' where uid='$examid'"))
{
	echo "Rank status set  - Rank will display at home page";
}else{
	echo mysqli_error($con);
}
}

$sql2=mysqli_query($con,"select * from `$que_ans_list` order by marks desc");
	...
	if($total_candidate2>0)
	{
		...
		?>
		<table border="1">
		<tr><th></th><th>Id</th><th>Name</th><th>Rank</th><th>Marks</th><th>Negative Mark</th><th>No Of Unsolved Question</th></tr>
		...
		while($row2=mysqli_fetch_assoc($sql2))
		{
			...
			$user_id=$row2['user_id'];
			 ...
			 $negative_mark=$row2['negative_mark'];
			$marks=$row2['marks'];
$sql_get_user=mysqli_query($con,"select name,crop_pic from allmember where id='$user_id'");
			if(mysqli_num_rows($sql_get_user)>0)
			{
			...
			if(mysqli_query($con,"update `$que_ans_list` set rank='$rank_user' where user_id='$user_id'"))
			{
				?>
				<tr>
				<?php if($crop_pic=='0' || $crop_pic==null){
					$crop_pic="default.jpg";
				}?>
				<td><img src="../../upload_images/crop/<?php echo $crop_pic; ?>"width="50"></td>
