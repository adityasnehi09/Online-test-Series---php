...
if(isset($_GET['change_series_no']))
	
	{
		...
		if(mysqli_query($con,"update all_test set series_no ='$series_no_' where uid='$examid_'"))
				{
					header("location: exam_control.php?status=serial changed");
	...
	
	{
		...
		if(mysqli_query($con,"update all_test set instruction ='$link_' where uid='$examid_'"))
				{
					header("location: exam_control.php?status=instruction_link_updated");
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Exam Control Center</h1>
					...
					if(isset($_REQUEST['exam_u_id']))
					{
						...
						$sqq=mysqli_query($con,"select uid,exam_name,starting_time,exam_for_display,starting_month,instruction,starting_year,starting_day,series_no,question_approve,ready_to_run,running_status from all_test  where uid='$exam_u_id'");
					}
					else{
					$sqq=mysqli_query($con,"select uid,exam_name,starting_time,exam_for_display,starting_month,instruction,starting_year,starting_day,series_no,question_approve,ready_to_run,running_status from all_test  order by s_no desc limit 0,1");
									...
									?>
										<?php if($exam_for_display==1)
									{
										?>
										<a href="change_exam_display_status.php?id=<?php echo $exam_uid;?>&action=0"><button class="btn btn-success" style="width:100%;margin-bottom:10px">Stop Exam Display</button></a>
										<?php
									}
									else
									{
										?>
										<a href="change_exam_display_status.php?id=<?php echo $exam_uid;?>&action=1"><button class="btn btn-danger" style="width:100%;margin-bottom:10px">Display Exam</button></a>
										<?php
									}
									?>
	...
	<form action="" method="get">
	  ...
	  <input type="hidden"name="examid_" value="<?php echo $exam_uid; ?>"/>
        <input type="submit" class="btn btn-default" value="Set Series"name="change_ins_link" />
								...
								<a href="edit_exam.php?exam_u_id=<?php echo $exam_uid;?>"><button class="btn btn-primary" style="width:100%;margin-bottom:10px">Edit Exam</button></a>
					...
					<table class="table">
                                ...
                                <?php 
									$sqq=mysqli_query($con,"select * from all_test order by s_no desc limit 0,10");
									if(mysqli_num_rows($sqq)>0)
									{
										?>
										...
										while($row=mysqli_fetch_assoc($sqq))
										{
											?><tr><td><?php echo $row['exam_name']; ?></td><td><?php echo $row['creating_time'].",".$row['creating_month'].":".$row['creating_day'].":".$row['creating_year'];?></td><td align="right"><a href="?exam_u_id=<?php echo $row['uid']; ?>"><button class="btn btn-default">Control</button></a></td></tr><?php
                            ...
                            <div class="list-group">
							 <?php 
									$sqq=mysqli_query($con,"select uid,exam_name,starting_time,starting_month,starting_year,starting_day from all_test  where complete_question='0' order by s_no desc limit 0,10");
									if(mysqli_num_rows($sqq)>0)
									{
										?>
										<?php
