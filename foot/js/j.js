...
// couple entry
function joincouple(comp_uid,version_uid,u)
{
	$("#partneremailmsg").html("<center><img src='img/fancybox_loading@2x.gif'class='img'></center>");
	
	var dataString = '&comp_uid='+ comp_uid +'&version_uid='+ version_uid + '&u='+ u;
	$.ajax({
type: "POST",
data: dataString,
url: "joincouple.php",
...
success: function(result){
	if(result=='01'){
		$("#partneremailmsg").html("Your partner is not a member of Night Bird");
	}
	else if(result=='0'){
...
cache: false,
success: function(result){
			$("#generalheading").html("Enter Email Id or Mobile no of your Partner");
				$("#generalbody").html(result);
...
function submitvideolink(comp_uid,video_id)
{
        ...
        if (!regexp.test(url)) {
				$("#successnotification").html("<b class='red-color'>Invalid URL : - Please Enter Valid URL</b>");
            notibox();
        } else {
           $("#successnotification").html("<b class='white-color'>Loading... Please wait !</b>");
            notibox();
          ...
          cache: false,
          success: function(result){
           $("#successnotification").html("<b class='white-color'>"+result+"</b>");
            notibox();
...
var txt = "";
    if (x.files.length == 0) {
        txt = "<b class='red-color'>Please Select a Audio file</b>";
    } else {
        ...
        for (var i = 0; i < x.files.length; i++) {
           ...
           if ('name' in file) {
				...
				if(fileExt=="MP3" || fileExt=="mp3" ||fileExt=="m4a" || fileExt=="M4A")
				{
					 ...
					 txt=" <b class='red-color'>Invalid format</b>";
				}
            }
            if ('size' in file) {
                ...
                txt=" <b class='red-color'>The size of file must be less then 20 MB </b>";
            }
	...
	$(document).ready(function(){
if(txt==""){txt=" <b class='white-color'>Please Wait...</b>";
var formdata = new FormData();
	...
	ajax.send(formdata);
$("#generalcropheading").html("<b class='white-color'>Loading ...</b> ");
