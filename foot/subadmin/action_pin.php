...
if(($amount * $list) > admin_info($_SESSION['USERID'],'balance')){
  die(display_flash("Your Account Balance Low!"));
}else{ 
 UPDATE('subadmin',array('balance' => (admin_info($_SESSION['USERID'],'balance') - ($amount * $list))),array('id'=>$_SESSION['USERID'])); 
 }
