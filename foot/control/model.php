...
require_once("welcome.php");
	$sql=mysql_query("select name from singing_june where email='$emailX'");
	$countsinging=mysql_num_rows($sql);
	$sql=mysql_query("select name from dance_june where email='$emailX'");
	$countdance=mysql_num_rows($sql);
	$sql=mysql_query("select name from acting_june where email='$emailX'");
	$countacting=mysql_num_rows($sql);
	$sql=mysql_query("select name from voice_june where email='$emailX'");
	$countvoice=mysql_num_rows($sql);
	?>
...
<div id="freeversion" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
  ...
  <div class="modal-content">
	...
	<?php
	// change per month
	$rockdance=mysql_query("select name from dance_june"); // change per month
	$rockacting=mysql_query("select name from acting_june"); // change per month
	$rockvoice=mysql_query("select name from voice_june"); // change per month
	$rocksinging=mysql_query("select name from singing_june"); // change per month
	$dance_can=mysql_num_rows($rockdance);
	$acting_can=mysql_num_rows($rockacting);
	$singing_can=mysql_num_rows($rocksinging);
	$voice_can=mysql_num_rows($rockvoice);
