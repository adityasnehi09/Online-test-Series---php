 ...
 if(!isset($_SESSION['adityasnehiloginyes']))
 {
	 header("location: http://nightbird.in");
	   ...
	   $resultsql=mysql_query("select * from allcompetition order by s_no desc");
	   
	  $countcersion= mysql_num_rows($resultsql);
	   ...
	   if($countcersion>0)
	   {
		  ...
		  <center>  <input type="button"class="btn  btn-small"style="outline:none"value="Learn More"onclick="learn('<?php echo $id ; ?>')"></a></center>
		   ...
		   require_once("welcome.php");
		   $sql33=mysql_query("select comp_name,uid from `$id`");
		   if(mysql_num_rows($sql33)>0)
		   {
			   while($srow=mysql_fetch_assoc($sql33))
			   {
				   $c=$srow['uid'];
				   echo "<a href='reg_p.php?v=$id&c=$c'>".$srow['comp_name']."</a>";
				   $ccc=mysql_query("select s_no from `$c`");
				   echo " ( member : ".mysql_num_rows($ccc) .")<br>";
			   }
		   }
	   }
