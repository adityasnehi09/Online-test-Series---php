...
<?php require_once("start_html_head.php"); ?>
		<title>IndoTufan | All India Online Jee Advance Test Series</title>
	    <meta name="description" content="Participate in All India Online JEE Advance Test Series and boost your capacity. ">
	    <meta name="keywords" content="indotufan.com,indotufan,jee Advance,indo tufan,test,test series,iit,online,online test series,All India Online Jee mains Test Series">	 
		<script>
...
function long_page(page_id){   
	...
	<div class="container" >
		...
		if(!isset($_SESSION['logintemp']))
		{
			...
			<div class="row overlay">
				<div class="col-lg-12 ">
					<div class="nightbird">
						<h1>IndoTufan | All India Online JEE Advance Test Series</h1>
						<h3>Test Series for JEE Advance</h3>
	...
	if(!isset($_SESSION['logintemp']))
		{
			...
			<section style="background-color:#213948;padding-top:50px;padding-bottom:50px;"id="aboutheading">
				<div class="container">
					...
					$ss=mysqli_query($con,"select * from all_test where exam_type='jee_advance_series' or exam_type='jee_advance_support' order by s_no desc limit 5");
					...
					require_once("welcome.php");
					$sql_rank=mysqli_query($con,"select uid from all_test where rank_status='1' and (exam_type='jee_advance_series' or exam_type='jee_advance_support') order by s_no desc limit 0,1");
  ...
  <!--/row-->
   ...
   <div class="container">
	...
	$sq=mysqli_query($con,"select * from all_test where uid='$exam_uid' and exam_for_display='1' and (exam_type='jee_advance_series' or exam_type='jee_advance_support')");
	}else{
		$sq=mysqli_query($con,"select * from all_test where exam_for_display='1' and ready_to_run='1' and (exam_type='jee_advance_series' or exam_type='jee_advance_support') order by s_no desc limit 0,1");
	}
...
?>
				...
				<div class="col-md-4">
				 <h3>Future JEE Advance Exams</h3>
				 <table class="table">
<div class="list-group">
<?php
				  $sqq=mysqli_query($con,"select * from all_test where exam_type='jee_advance_series' or exam_type='jee_advance_support' order by s_no desc limit 10");
				 $series_no=mysqli_num_rows($sqq);
				 if($series_no>0)
				 {
					 while($row=mysqli_fetch_assoc($sqq))
