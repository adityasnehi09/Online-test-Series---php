...
<?php require_once("start_html_head.php"); ?>
		<title>IndoTufan | All India Online Gate Test Series</title>
		 <meta property="og:site_name" content="IndoTufan">
    
    <meta property="og:title" content="IndoTufan | All India Online Gate Test Series">
    <meta property="og:image" content="img/success2.jpg">
	   ...
	   <meta name="description" content="Participate in All India Online Gate Test Series and boost your capacity. ">
	    <meta name="keywords" content="indotufan.com,indotufan,indo tufan,test,test series,online,online test series,All India Online Gate Test Series,aits, AITS">	 
	...
	<div class="row overlay">
				...
				<a href="?type=cs"><div class="btn btn-success col-xs-12 col-sm-4 col-md-3" style="margin-bottom:10px;">Computer Science Engg</div></a>
				<a href="?type=ec"><div class="btn btn-success col-xs-12 col-sm-4 col-md-3" style="margin-bottom:10px;">Electronics Engg</div></a>
				<a href="?type=ee"><div class="btn btn-success col-xs-12 col-sm-4 col-md-3" style="margin-bottom:10px;">Electrical Engg</div></a>
				<a href="?type=ce"><div class="btn btn-success col-xs-12 col-sm-4 col-md-3" style="margin-bottom:10px;">Civil Engg</div></a>
				<a href="?type=me"><div class="btn btn-success col-xs-12 col-sm-4 col-md-3" style="margin-bottom:10px;">Mechanical Engg</div></a>
				<a href="?type=ma"><div class="btn btn-success col-xs-12 col-sm-4 col-md-3" style="margin-bottom:10px;">Engineering Mathematics</div></a>
				<a href="?type=ga"><div class="btn btn-success col-xs-12 col-sm-4 col-md-3"  style="margin-bottom:10px;">General Aptitude</div></a>
	...
	if(isset($_REQUEST['type'])){
			$type=securedata($_REQUEST['type']);
			$sql=mysqli_query($con,"select * from gate_category where code_word='$type'");
			if(mysqli_num_rows($sql)>0){
				while($row_=mysqli_fetch_assoc($sql)){
					$stream_name=$row_['name'];
					$steam_name=$row_['link_of_syllabus'];
					?><h1 style="font-family: 'Yanone Kaffeesatz', sans-serif;"><?php echo $stream_name; ?></h1> <?php
					?>
	...
	if(isset($_REQUEST['id'])){
		...
		$sql2=mysqli_query($con,"select * from all_test where exam_for_display='1' and ready_to_run='1' and streams='$type' and uid='$id__'order by s_no desc limit 0,1");
	
	}else{
		$sql2=mysqli_query($con,"select * from all_test where exam_for_display='1' and ready_to_run='1' and streams='$type' order by s_no desc limit 0,1");
...
$startfromthistime= round(abs($today_date - $start_date ) / 60,2);	
   ...
   <div class="col-md-4">
   <?php if($type=='cs'){?>
   <a href="syllabus/Syllabus_and_Schedule-All_India_Gate_Test_Series-2018.pdf"><button class="btn btn-primary"style="width:100%">View Timetable and syllabus</button></a>
   <?php  } ?>
<?php if($type=='me'){?>
   <a href="IndoTufan_All_India_Test_series_for_GATE_Mechanical_Engineering_SYLLABUS.pdf"><button class="btn btn-primary"style="width:100%">View Timetable and syllabus</button></a>
   <?php  } ?>   
...
<?php
				  $sqq=mysqli_query($con,"select * from all_test where streams='$type' and exam_for_display='1' order by s_no desc limit 10");
	...
	<br>
			<section style="background-color:#213948;padding-top:50px;padding-bottom:50px;"id="aboutheading">
				<div class="container">
					...
					require_once("welcome.php");
					$ss=mysqli_query($con,"select * from all_test where exam_for_display='1' and (exam_type='gate_series' or exam_type='gate_support') order by s_no desc ");
					...
					require_once("welcome.php");
					$sql_rank=mysqli_query($con,"select uid from all_test where rank_status='1' and (exam_type='gate_series' or exam_type='gate_support') order by s_no desc limit 0,1");
